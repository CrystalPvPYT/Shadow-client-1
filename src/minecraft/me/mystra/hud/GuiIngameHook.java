...
import me.mystra.management.animate.AnimationUtil;
import me.mystra.module.Module;
import me.mystra.utils.render.TTFFontRenderer;
...
public class GuiIngameHook extends GuiIngame {
	...
	private final static ResourceLocation MYSTRA = new ResourceLocation("mystra/Mystra32x32.png");
	...
	public static void HUDArrayList() {
		...
		if(mode.equalsIgnoreCase("Novoline")) {
			...
			Mystra.instance.moduleManager.getModules().stream().filter(Module::isToggled).sorted(Comparator.comparingInt(module -> -font.getStringWidth(module.getDisplayName()))).forEach(module -> {
				...
				if(Mystra.instance.settingsManager.getSettingByName("ArrayListFont").getValBoolean()) {
					Mystra.fontManager.getFont("SFB 12").drawString(module.getDisplayName(), sr.getScaledWidth() - 3 - font.getStringWidth(module.getDisplayName()), yDist[0] - 1, novoline(counter[0] * 300));
					Mystra.fontManager.getFont("SFB 12").drawString(module.getDisplayName(), sr.getScaledWidth() - 3.2F - font.getStringWidth(module.getDisplayName()), yDist[0] - 1, novoline(counter[0] * 300));
				} else {
	...
	public static void mystraname() {
		String themeadv = Mystra.instance.settingsManager.getSettingByName("Theme").getValString();
		int counter = 0;
        final ScaledResolution sr = new ScaledResolution(Minecraft.getMinecraft());
        if(Mystra.instance.settingsManager.getSettingByName("Watermark").getValBoolean()) {
        	if(themeadv.equalsIgnoreCase("Mystra")) {
        		//Mystra.instance.fontManager.getFont("SF-UI-Display-Bold").drawStringWithMystra("ï¿½lMystra ï¿½rï¿½7ï¿½lBï¿½oï¿½l1", 10.0f, 3.0f, rainbow(counter*300));
        		Mystra.instance.fontManager.getFont("SFB 10").drawStringWithShadow(Mystra.instance.uiname.substring(0), 12.0f, 3.0f, rainbow(counter*300));
        	}
        	
        	if(themeadv.equalsIgnoreCase("Cheese")) {
        		Mystra.instance.fontManager.getFont("SFB 10").drawStringWithShadow("§eCheezy Boi", 7, 12 - 10, rainbow(counter*300));
        	}
