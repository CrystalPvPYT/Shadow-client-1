...
public class MystraTabGui {
    ...
    public void drawTabGui() {
        ...
        for(Category c : Category.values()) {
            if(!c.name().equalsIgnoreCase("Gui")) {
                ...
                String colortheme = Mystra.instance.settingsManager.getSettingByName("Theme").getValString();
                if(colortheme.equalsIgnoreCase("Mystra")) {
        ...
        for(String s : this.category) {
        	Mystra.instance.fontManager.getFont("SFB 8").drawCenteredString(s, 35, 17 + categoryCount * 13, -1);
            categoryCount++;
        }
        if(screen == 1) {
            int modCount = 0;
    ...
    private ArrayList<Module> getModsForCategory() {
        ...
        for(Module mod : Mystra.instance.moduleManager.getModules()) {
