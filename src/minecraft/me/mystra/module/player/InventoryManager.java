...
import me.mystra.utils.InventoryUtils;
...
public final class InventoryManager extends Module {
   ...
   public InventoryManager() {
		...
		Mystra.instance.settingsManager.rSetting(new Setting("Sword Slot", this, 1.0D, 1.0D, 10.0D, true));
		 Mystra.instance.settingsManager.rSetting(new Setting("InvManager Delay", this, 250.0D, 0.0D, 1000.0D, true));
	}
...
public static final TimeHelper INV_STOPWATCH = new TimeHelper();
	...
	public void onPreMotion(EventUpdate event) {
      if ((mc.currentScreen == null || mc.currentScreen instanceof GuiInventory)) {
         ...
         if (bestSwordSlot != -1 && !this.checkDelay()) {
            mc.playerController.windowClick(windowId, bestSwordSlot < 9 ? bestSwordSlot + 36 : bestSwordSlot, (int) ((Mystra.instance.settingsManager.getSettingByName("Sword Slot").getValDouble()) - 1), 2, player);
            INV_STOPWATCH.reset();
         }
      }
   ...
   private boolean checkDelay() {
      return !INV_STOPWATCH.hasReached((long) Mystra.instance.settingsManager.getSettingByName("InvManager Delay").getValDouble());
   }
