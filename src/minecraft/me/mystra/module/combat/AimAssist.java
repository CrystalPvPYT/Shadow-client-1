...
public class AimAssist extends Module {
	...
	public AimAssist() {
		...
		Mystra.instance.settingsManager.rSetting(new Setting("FOV", this, 70.0D, 30.0D, 180.0D, false));
		Mystra.instance.settingsManager.rSetting(new Setting("Speed Divider", this, 35.0D, 1.0D, 75.0D, false));
		Mystra.instance.settingsManager.rSetting(new Setting("SWORD", this, false));
	...
	public void onTick(ClientTickEvent e)
	   {
	     if (mc.theWorld != null) {
	       if (Mystra.instance.settingsManager.getSettingByName("SWORD").getValBoolean()) {
	       ...
	       if ((target != null) && ((getDistanceToCrosshair(target) > 1.0D) || (getDistanceToCrosshair(target) < -1.0D))) {
	         boolean left = getDistanceToCrosshair(target) > 0.0D; EntityPlayerSP 
	           tmp155_152 = mc.thePlayer;tmp155_152.rotationYaw = ((float)(tmp155_152.rotationYaw + (left ? -(Math.abs(getDistanceToCrosshair(target)) / Mystra.instance.settingsManager.getSettingByName("Speed Divider").getValDouble()) : Math.abs(getDistanceToCrosshair(target)) / Mystra.instance.settingsManager.getSettingByName("Speed Divider").getValDouble())));
	       }
	     }
	   }
	   ...
	   public Entity findBestTarget() {
	     Entity e = null;
	     int fov = (int)Mystra.instance.settingsManager.getSettingByName("FOV").getValDouble();
	   ...
	   public boolean isANigger(Entity e) {
	     if (((e instanceof EntityLivingBase)) && 
	       (((EntityLivingBase)e).getTeam() != null) && (((EntityLivingBase)e).isOnSameTeam(mc.thePlayer)) && (Mystra.instance.settingsManager.getSettingByName("TEAMS").getValBoolean())) {
	       return false;
	     }
