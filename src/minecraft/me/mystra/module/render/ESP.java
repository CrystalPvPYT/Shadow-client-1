...
import me.mystra.module.combat.Aura;
...
public class ESP extends Module{
	...
	public ESP() {
        ...
        Mystra.instance.settingsManager.rSetting(new Setting("ESP Mode", this, "Corner", options));
		//Mystra.instance.settingsManager.rSetting(new Setting("Chest", this, true));
		Mystra.instance.settingsManager.rSetting(new Setting("Rainbow", this, true));
		Mystra.instance.settingsManager.rSetting(new Setting("Animate", this, true));
	}

	public void onRender() {
		if(this.isToggled()){
			String mode = Mystra.instance.settingsManager.getSettingByName("ESP Mode").getValString();
			...
			if(mode.equalsIgnoreCase("Box")) {
				for (final Object o : this.mc.theWorld.loadedEntityList) {
	                if (o instanceof EntityPlayer && o != this.mc.thePlayer && o != null) {
	                    final EntityPlayer e = (EntityPlayer)o;
	                    RenderUtils.esp(e, this, Mystra.instance.settingsManager.getSettingByName("Rainbow").getValBoolean());
	                }
	            }
			}
			...
			if(mode.equalsIgnoreCase("Penis")) {
				for (final Object o : mc.theWorld.loadedEntityList) {
		            ...
		            if (Mystra.instance.settingsManager.getSettingByName("Animate").getValBoolean()) {
		                ++this.amount;
		                if (this.amount > 25) {
		                    ++this.spin;
		                    if (this.spin > 50.0f) {
			...
			if(mode.equalsIgnoreCase("Outline2")) {
				...
				if(!(o instanceof EntityPlayerSP)) {
                    if(o instanceof EntityPlayer) {
                        EntityPlayer en = (EntityPlayer)o;
                        if(Mystra.instance.friendManager.isFriend(en.getName())) {
	...
	private void doCornerESP() {
		...
		while (var2.hasNext()) {
			...
			if (entity != this.mc.thePlayer) {
				...
				} else if (entity == Aura.ThisIsTheEntityThatThePlayerIsHittingTo && Mystra.instance.moduleManager.getModuleByName("Aura").isToggled()) {
	...
	private void doOutlineESP(EventRendererLivingEntity event) {
		if (this.mc.thePlayer != event.entity) {
			if (this.isValid(event.entity)) {
				...
				} else if (event.entity == Aura.ThisIsTheEntityThatThePlayerIsHittingTo && Mystra.instance.moduleManager.getModuleByName("Aura").isToggled()) {
	...
	private boolean isValid(EntityLivingBase entity) {
		return entity == this.mc.thePlayer ? false
				: (entity.getHealth() <= 0.0F ? false
						: (entity instanceof EntityPlayer && ((Boolean) Mystra.instance.settingsManager.getSettingByName("Players").getValBoolean()
								? true
								: (entity instanceof EntityAnimal
										&& ((Boolean) Mystra.instance.settingsManager.getSettingByName("Animals").getValBoolean() ? true
												: entity instanceof EntityMob
														&& ((Boolean) Mystra.instance.settingsManager.getSettingByName("Mobs").getValBoolean()))))));
	}
