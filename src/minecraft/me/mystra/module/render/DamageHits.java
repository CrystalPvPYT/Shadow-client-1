...
public class DamageHits extends Module {
	...
	public void onRender(Event3D event) {
			for (final DamageText text : new ArrayList<DamageText>(damageTexts)) {
				...
				Mystra.fontManager.getFont("SFL 10").drawString(text.getText(), -Mystra.fontManager.getFont("SFL 10").getWidth(text.getText()) / 2 + (text.isNegative() ? -text.getrX() : text.getrX()), (text.isNegative() ? -text.getrY() : text.getrY()), novoline(300));
	...
	class DamageText {
		...
		public void updateAlpha() {
			alpha -= Mystra.DELTA_UTIL.deltaTime * .25;
