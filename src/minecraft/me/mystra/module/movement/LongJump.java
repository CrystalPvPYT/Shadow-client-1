...
public class LongJump extends Module{
	...
	public LongJump() {
        ...
        //AAC
        options.add("AAC");
        Mystra.instance.settingsManager.rSetting(new Setting("LongJump Mode", this, "Hypixel", options));

        Mystra.instance.settingsManager.rSetting(new Setting("Jump Glide", this, false));
        Mystra.instance.settingsManager.rSetting(new Setting("Jump Disable", this, false));
	}

	@EventTarget
	public void onUpdate(EventUpdate event) {
		if(this.isToggled()){
			String mode = Mystra.instance.settingsManager.getSettingByName("LongJump Mode").getValString();
	        ...
	        if(mode.equalsIgnoreCase("Hypixel")) {
	            ...
	            if ((mc.thePlayer.moveForward != 0 || mc.thePlayer.moveStrafing != 0) && mc.thePlayer.onGround) {
	                if(Mystra.instance.settingsManager.getSettingByName("Jump Disable").getValBoolean() && groundTicks > 0){
	                    groundTicks = 0;
	                    this.toggle();
	                    return;
	                }
	            ...
	            if (isOnGround(0.001)) {
	                ...
	                if(mc.thePlayer.isCollidedVertically)
	                    stage = 0;
	                if(stage > 0 && stage <= 3 && Mystra.instance.settingsManager.getSettingByName("Jump Glide").getValBoolean()){
	                    mc.thePlayer.onGround = true;
	                }
	                double speed = (0.95 + getSpeedEffect() * 0.2) - air / 25;
	                if(Mystra.instance.settingsManager.getSettingByName("Jump Glide").getValBoolean()){
	                    speed = (1.1 + getSpeedEffect() * 0.2f) - air / 20;
	                }
	                if(speed < defaultSpeed()-0.05){ // + (0.025*MoveUtils.getSpeedEffect())
	                    speed = defaultSpeed()-0.05;
	                }
	                if(stage < 4 && Mystra.instance.settingsManager.getSettingByName("Jump Glide").getValBoolean())
	                    speed = defaultSpeed();
	                setMotion(speed);
	                if(Mystra.instance.settingsManager.getSettingByName("Jump Glide").getValBoolean()){
