package me.mystra.gui.altmanager;
...
import me.mystra.gui.altmanager.components.GuiAccountList;
import me.mystra.gui.altmanager.impl.GuiAddAlt;
import me.mystra.gui.altmanager.impl.GuiAltLogin;
...
public class GuiAltManager extends GuiScreen {
	...
	public void drawScreen(int p_drawScreen_1_, int p_drawScreen_2_, float p_drawScreen_3_) {
		...
		if (Mystra.instance.getAltService().getCurrentService().equals(AltService.EnumAltService.MOJANG)) {
			drawCenteredString(this.mc.fontRendererObj, Strings.translateColors(status), scaledResolution.getScaledWidth() / 2, 6, -3158065);
			drawCenteredString(this.mc.fontRendererObj, "Accounts: " + Mystra.instance.getAccountManager().getAccounts().size(), this.width / 2, 20, -1);
		} else
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		case 6:
			...
			break;
		case 7:
			if (Mystra.instance.getAccountManager().getAccounts().size() == 0)
				return;

			registry = Mystra.instance.getAccountManager().getAccounts();
			random = new Random();
			randomAlt = registry.get(random.nextInt(Mystra.instance.getAccountManager().getAccounts().size()));
		...
		case 5:
			...
			if (returnVal == 0) {
				...
				try {
					for (String line : Files.readAllLines(Paths.get(chooser.getSelectedFile().getPath(), new String[0]))) {
						...
						Mystra.instance.getAccountManager().getAccounts().add(account);
					}
			...
			break;
		case 8:
			if (Mystra.instance.getAccountManager().getLastAlt() == null)
				return;
			this.loginThread = new AccountLoginThread(Mystra.instance.getAccountManager().getLastAlt().getEmail(), Mystra.instance.getAccountManager().getLastAlt().getPassword());
			this.loginThread.start();
			break;
		case 9:
			if (Mystra.instance.getAccountManager().getAccounts().isEmpty())
				return;
			Mystra.instance.getAccountManager().getAccounts().clear();
			break;
		}
	}
