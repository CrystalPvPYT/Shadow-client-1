package me.mystra.gui.login.elements.buttons;
...
import me.mystra.utils.gui.easings.utilities.Progression;
...
public class GuiTexturedButton extends GuiButton {
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY) {
		Mystra.RENDER2D.color(Color.white);
		updateButton(mouseX, mouseY);
		
		Mystra.RENDER2D.push();
		if (scissor) {
			GL11.glEnable(GL11.GL_SCISSOR_TEST);
			Mystra.RENDER2D.scissor(startPosX, startPosY + 1, width, height - 1);
		}
		Mystra.RENDER2D.translate(this.xPosition + this.width / 2, this.yPosition + this.height / 2);
		Mystra.RENDER2D.rect(-width / 2, -height / 2, width, height, new Color(0, 0, 0, 200));
		Mystra.RENDER2D.scale(1 + scaleFactor, 1 + scaleFactor);
		Mystra.RENDER2D.image(resourceLocation, -this.width / 2 + 10, -this.height / 2 + 10, this.width - 20, this.height - 20);
		if (scissor) GL11.glDisable(GL11.GL_SCISSOR_TEST);
		Mystra.RENDER2D.pop();
		if (!displayString.isEmpty())
			Mystra.fontManager.getFont("SFL 10").drawStringWithShadow(displayString, xPosition + width / 2 - Mystra.fontManager.getFont("SFL 10").getWidth(displayString) /  2, yPosition + height / 2 - Mystra.fontManager.getFont("SFL 10").getHeight(displayString) / 2, -1);
		if (scissor) Mystra.RENDER2D.outlineInlinedGradientRect(startPosX, startPosY, width, height, 5, Color.black, new Color(0, 0, 0, 0));
	}
